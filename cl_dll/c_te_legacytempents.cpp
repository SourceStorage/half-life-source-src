...
void CTempEnts::MuzzleFlash( const Vector& pos1, const QAngle& angles, int type, int entityIndex, bool firstPerson )
{
	...
	frameCount = modelinfo->GetModelFrameCount(m_pHL1SpriteMuzzleFlash[index]);

	
	pTemp = TempEntAlloc(pos1, (model_t *)m_pHL1SpriteMuzzleFlash[index]);
	if (!pTemp)
		return;
...
void CTempEnts::LevelInit()
{
	...
	m_pHL1SpriteMuzzleFlash[0] = (model_t *)engine->LoadModel("sprites/muzzleflash1.vmt");
	m_pHL1SpriteMuzzleFlash[1] = (model_t *)engine->LoadModel("sprites/muzzleflash2.vmt");
	m_pHL1SpriteMuzzleFlash[2] = (model_t *)engine->LoadModel("sprites/muzzleflash3.vmt");
